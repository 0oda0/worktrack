...
## Воркфлоу каждого спринта (фиксированный)

1. **Реализация** — расчётная логика через TDD (сначала тест, потом код).
2. **Тестирование** — `pytest` зелёный + ручная проверка эндпоинтов (curl/docs).
3. **Код-ревью** — ревью сделанного кода (свежим взглядом: корректность, права
   доступа, часовые пояса, краевые случаи), правки по итогам.
4. **Коммит** — один коммит на спринт с осмысленным сообщением.
5. **СТОП** — жду команду «дальше» от пользователя. Без неё следующий спринт не начинается.
